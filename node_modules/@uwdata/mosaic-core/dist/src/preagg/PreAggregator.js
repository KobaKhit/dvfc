import { Query, TableRefNode, createSchema, and, asNode, ceil, collectColumns, createTable, float64, floor, isBetween, int32, mul, round, scaleTransform, sub, isSelectQuery, isAggregateExpression, ColumnNameRefNode, rewrite } from '@uwdata/mosaic-sql';
...
function activeColumns(clause) {
    ...
    if (meta.type === 'point') {
        ...
        predicate = (clause) => {
            if (!clause?.predicate)
                return [];
            const map = new Map(clause.fields.map((f, i) => [f, asNode(`active${i}`)]));
            ...
            if (bins.length === 1) {
                ...
                predicate = (clause) => {
                    if (!clause?.predicate)
                        return [];
                    const v = clause.value;
                    return and(fields.map((_, i) => isBetween(`active${i}`, v[i].map(bins[i]))));
                };
            }
        }
    }
 ...
 */
function replaceIndices(exprs, select) {
