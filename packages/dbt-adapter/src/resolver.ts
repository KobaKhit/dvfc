...
export class DbtResolver {
  ...
  private pathResolver?: (node: DbtNode) => string;

  constructor(options: DbtResolverOptions | string) {
    if (typeof options === 'string') {
      // Legacy simple constructor
      throw new Error(
        'DbtResolver now requires options object. Use: new DbtResolver({ manifestPath, dataDir })'
      );
    }
   ...
   */
  async loadManifest(manifestData: DbtManifest): Promise<void> {
   ...
   */
  private findModel(modelName: string): DbtNode | undefined {
 ...
 */
export async function createDbtResolver(
  manifestData: DbtManifest,
  options: Omit<DbtResolverOptions, 'manifestPath'>
): Promise<DbtResolver> {
